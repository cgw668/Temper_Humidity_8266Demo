...
ble_sts_t att_sendMsg(u16 connHandle, u8* param, u8 paramLen);


ble_sts_t att_verifyAuthSig(u8* pData);


void att_getLongUUID(uuid_t* uuid);


u8 att_uuidCompare(uuid_t* uuid_1, uuid_t* uuid_2);

ble_sts_t att_getExchangeMtuReq(u8* pData, u8 len, exchangeMTUReq_t* pReq);


ble_sts_t att_sendExchangeMtuReq(u16 connHandle, exchangeMTUReq_t* pReq);


ble_sts_t att_getExchangeMtuRsp(u8* pData, u8 len, exchangeMTURsp_t* pRsp);


ble_sts_t att_sendExchangeMtuRsp(u16 connHandle, exchangeMTURsp_t* rsp, u8* buffer);


ble_sts_t att_getFindInfoReq(u8* pData, u8 len, findInformationReq_t* pReq);


ble_sts_t att_sendFindInfoRsp(u16 connHandle, findInformationRsp_t* pRsp, u8* buffer);


ble_sts_t att_getReadReq(u8* pData, u8 len, readReq_t* pReq);


ble_sts_t att_sendReadReq(u16 connHandle, readReq_t* pReq);


ble_sts_t att_getReadRsp(u8* pData, u8 len, readRsp_t* pRsp);


ble_sts_t att_sendReadRsp(u16 connHandle, readRsp_t* pRsp, u8* buffer);


ble_sts_t att_getReadBlobReq(u8* pData, u8 len, readBlobReq_t* pReq);


ble_sts_t att_sendReadBlobRsp(u16 connHandle, readBlobRsp_t* pRsp, u8* buffer);


ble_sts_t att_getReadByTypeReq(u8* pData, u8 len, readByTypeReq_t* pReq);


ble_sts_t att_sendReadByTypeRsp(u16 connHandle, readByTypeRsp_t* pRsp, u8* buffer);


ble_sts_t att_getReadByGroupTypeReq(u8* pData, u8 len, readByGroupTypeReq_t* pReq);


ble_sts_t att_sendReadByGroupTypeReq(u16 connHandle, readByGroupTypeReq_t* pReq);


ble_sts_t att_getReadByGroupTypeRsp(u8* pData, u8 len, readByGroupTypeRsp_t* pRsp);


ble_sts_t att_sendReadByGroupTypeRsp(u16 connHandle, readByGroupTypeRsp_t* rsp, u8* buffer);


ble_sts_t att_getWriteReq(u8* pData, u8 len, writeReq_t* pReq);


ble_sts_t att_sendWriteReq(u16 connHandle, writeReq_t* pReq);


ble_sts_t att_sendWriteCmd(u16 connHandle, writeCmd_t* pReq);


ble_sts_t att_sendWriteRsp(u16 connHandle, u8* buffer);


ble_sts_t att_getErrorRsp(u8* pData, u8 len, errorRsp_t* pRsp);


ble_sts_t att_sendErrorRsp(u16 connHandle, errorRsp_t* rsp, u8* buffer);


ble_sts_t att_handleValueNoti(u16 connHandle, handleValueNoti_t *pNoti);
